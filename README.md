...
| `130` | Run cancelled (Ctrl-C aborts the run cooperatively)     |
...
// A signal lets a caller cancel the run cooperatively; pass an AbortController's signal.
...
Progress flows through the `ProgressEmitter` as typed events (`phaseStart`,
`directoryStart`, `file`, `counts`) — `CliReporter` renders them as spinner
lines, and any other consumer (e.g. an Electron renderer) can subscribe to the
same channel for a determinate progress bar.
