...
- **`src/`** - All TypeScript source files live here
  ...
  - `phases/` - Phase classes (`scan-phase.ts`, `resync-phase.ts`, `records-phase.ts`), the `Phase`/`PhaseContext`/`PhaseResult` contract, the `abort.ts` cancellation primitive, and a shared `iterate-directories.ts`
  - `output/` - `reporter.ts` (CliReporter, the only console user) and `progress.ts` (typed ProgressEmitter)
  - `services/` - `db-service.ts` (SQLite) and `file-service.ts` (file listing/hashing)
  - `types/` - `configuration.ts`, `file-types.ts`, `run-summary.ts`, and `progress.ts` (ProgressEvent/ProgressSink contract)
  - `utilities/` - `load-config.ts` (Zod-validated YAML config loader), `error-message.ts`, and `path-helpers.ts`
